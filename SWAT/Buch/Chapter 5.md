**Give a reason why it is a good practice to limit the number of characteristics an architecture should support**
Die unterstüzten Charakteriskiten sollten sich auf ein Minimum beschränken, da mit jeder Charakteristik das System komplexer wird.

**True of False. Most architecture characteristics com from business requirements and user stories**
True

**If a business stakeholder states that time to market is the most important business concern, which architecture characteristics would the architecture need to support?**
Vorallem Agilität -> Das ist aber auch einen zusammengesetzte Eigenschaft und beinhaltet z.B:
- Deployability
- Modularization
- Maintainability
- Testability 

**What is the difference between scalability and elasticity**
Scaleability beschreibt die Eigneschaft über "längere Zeit" mit Wachsenden benutzer mit zu gehen. Elasticity beschreibt dass auf Bursts reagiert werden kann.

**What is the definition of a composite architectural characteristic**
Beispiel Agility
Wenn eine eigenschaft aus mehreren verschiedenen "Sub-Charakteristiken" besteht.
